...
"use strict";
		...
		function toggleMobileLnb(event) {
			...
			if(event.target.parentNode.classList.contains(IS_OPEN)) {
				...
				event.target.nextElementSibling.style.maxHeight = event.target.nextElementSibling.children[0].children.length * lnbItemHeight + lnbItemPadding + 'px';
			}
		}
		...
		const burgerBtnline = document.querySelector('.burger_btn span');
		...
		burgerBtnline.addEventListener('click', toggleMobileNav);
